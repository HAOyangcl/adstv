...
const Duration kAutoHideCursorDuration = Duration(seconds: 3);
...
class TVUIState extends State<TVUI>
  ...
  void _hideCursorNative() {
    if (!mounted) return;
    // 使用 MouseRegion 包裹需要隐藏光标的区域
    // 这里通过设置状态来触发 UI 更新，由 MouseRegion 处理光标隐藏
    setState(() {
      _isCursorHidden = true;
    });
  }

  void _showCursorNative() {
    if (!mounted) return;
    setState(() {
 ...
 @override
  ...
  FutureOr<void> afterFirstLayout(BuildContext context) {
    initData();
    if (GetPlatform.isDesktop) _showCursorNative();
      ...
      children: [
        ...
        Positioned.fill(
          child: GestureDetector(
            ...
            child: MouseRegion(
              cursor: _isCursorHidden ? SystemMouseCursors.none : SystemMouseCursors.basic,
              onHover: hijackAutoHideCursor,
              child: SizedBox(
                width: double.infinity,
                height: double.infinity,
